...
%define _prefix      /opt
%define _bin_dir     %{_prefix}/%{name}/bin
%define _conf_dir    %{_prefix}/%{name}/conf
...
install -p -D -m 644 lib/*.jar $RPM_BUILD_ROOT%{_bin_dir}
...
CLASSPATH=
for i in $RPM_BUILD_ROOT%{_prefix}/zookeeper/bin/*.jar
do
  CLASSPATH="%{_bin_dir}/$(basename ${i}):${CLASSPATH}"
done
